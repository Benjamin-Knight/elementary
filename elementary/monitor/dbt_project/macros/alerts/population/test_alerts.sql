...
{% macro populate_test_alerts_query(days_back=1) %}
    ...
    select distinct
        ...
        coalesce(failed_tests.test_unique_id, 'None') {{ elementary.string_join() }} '.' {{ elementary.string_join() }} coalesce(failed_tests.column_name, 'None') {{ elementary.string_join() }} '.' {{ elementary.string_join() }} coalesce(failed_tests.sub_type, 'None') as alert_class_id,
        case
            ...
            else coalesce(failed_tests.test_unique_id, 'None') {{ elementary.string_join() }} '.' {{ elementary.string_join() }} coalesce(failed_tests.column_name, 'None') {{ elementary.string_join() }} '.' {{ elementary.string_join() }} coalesce(failed_tests.sub_type, 'None')
